...
def mail(data,time,pred_path):
    ret = True
    print(data)
    try:
        msg=MIMEText('填写邮件内容','plain','utf-8')
        msg['From'] = formataddr(["Joy", my_sender])  # 括号里的对应发件人邮箱昵称、发件人邮箱账号
        msg['To'] = formataddr(["FK", my_user])  # 括号里的对应收件人邮箱昵称、收件人邮箱账号
        msg['Subject'] = time  # 邮件的主题，也可以说是标题

        att = MIMEText(open(pred_path, 'rb').read(), 'base64', 'utf-8')
